...
RDEPEND=">=virtual/jre-1.6.0
	...
	mencoder? ( >media-video/mplayer-1.1.1-r1[dts,dvd,encode,faad,iconv,libass,mp3,theora,toolame,truetype,twolame,unicode,vorbis,x264,xvid] )
	ffmpeg? ( >=media-video/ffmpeg-2.2.2[aac,aacplus,bluray,encode,faac,fontconfig,iconv,libass,mp3,vorbis,x264,xvid,zlib] )"
...
src_install() {
	...
	dodir "/var/log/universalmediaserver/${UMS_USER}"
	...
	insinto "${UMS_HOME}"
	doins -r ums.jar documentation plugins renderers *.xml
...
pkg_postinst() {
	...
	chown -R "${UMS_USER}":"${UMS_GROUP}" "/var/log/universalmediaserver"
